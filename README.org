...
used to define data in memory. These **are not data types in the high-level language sense**,
but rather instructions to the assembler about how many bytes
...
+ Variadic functions :: Functions that accept a variable number of arguments,
  not a fixed number. For example, in C we have ~printf(const char *format, ...)~
